...
//                Category c = new Category(1, );
                ...
                trader.save(Trader.builder().name("Yenefa Ibrahin").birth((new Date())).contact("312223-73788497").passwd(encrypter("")).status(GeneralStatus.APPROVED).preferJson("{\"prefers\": []}").type(Type.CUSTOMER).registrar(relatedJaager).build());
                trader.save(Trader.builder().name("Bin Nsibi").birth((new Date())).contact("31313-73788466").passwd(encrypter("")).status(GeneralStatus.APPROVED).preferJson("{\"prefers\": []}").type(Type.CUSTOMER).registrar(relatedChulmann).build());
