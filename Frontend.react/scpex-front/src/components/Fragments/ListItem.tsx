...
export default class ListItem extends Component<ListItemProps>{
    ...
    render(): ReactNode {
        return (
            <div onClick={this.props.onClick ?? (() => {console.log("ListItem Received Click. ")})} className={`text-slate-950 dark:text-slate-50   rounded-lg h-full bg-slate-300 dark:bg-slate-700 flex flex-row shadow-md hover:shadow-lg overflow-hidden opacity-100 hover:opacity-90 active:opacity-80 transition`}>
                <div className="w-32 h-32 relative @COVER">
                    {/* <img src="" alt="" />  to prevent image stealing so gave up img */}
                    <div className={`w-32 h-32 z-[3] absolute`} style={{background: `url(${this.props.cover! as string})`}}></div>
                ...
                <div className="grow @INFO p-1.5 px-2 md:p-3 md:px-5 h-full relative flex flex-col">
